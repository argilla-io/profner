...
def brat2doc(
    ...
    **kwargs,
    ...
    replace_antibert_token_with
        Replaces antibert tokens (tokens that the BERT tokenizer just skipps) with this char.
        You also need to provide the `bert_tokenizer`!
    bert_tokenizer
        Used to replace antibert tokens
    **kwargs
    ...
    if len(doc) != len(tags):
        raise ValueError(f"Lengths of tokens and tags do not match: {doc} {tags}")
...
def get_custom_tokenizer_v1() -> spacy.language.Language:
    nlp = spacy.load("es_core_news_sm")

    # TODO: Check if this can be improved/generalized better, 
    # e.g. include some regex that covers all skin tones or something like that.
